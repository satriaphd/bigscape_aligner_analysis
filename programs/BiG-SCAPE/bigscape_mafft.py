...
def process_gbk_files(gbk, min_bgc_size, bgc_info, files_no_proteins, files_no_biosynthetic_genes):
    ...
    if os.path.isfile(outputfile) and os.path.getsize(outputfile) > 0 and not force_hmmscan:
        if verbose:
            print(str(datetime.now()) + ": " + " File {} already processed".format(outputfile))
        save_fasta = False
    else:
        save_fasta = True
    ...
    try:
        ...
        print(str(datetime.now()) + ": " + "   Error with file {}: \n    '{}'".format(gbk, str(e)))
        print(str(datetime.now()) + ": " + "    (This file will be excluded from the analysis)")
        return
    else:
        total_seq_length = 0
        bgc_size = 0
                ...
                if "cluster" in feature.type:
                    if "product" in feature.qualifiers:
                        if len(feature.qualifiers["product"]) > 1:
                            print(str(datetime.now()) + ": " + "  WARNING: more than product annotated in record " + str(record_count) + ", " + fname)
                            break
                        else:
                    ...
                    if "contig_edge" in feature.qualifiers:
                        ...
                        if feature.qualifiers["contig_edge"][0] == "True":
                            if verbose:
                                print(str(datetime.now()) + ": " + " Contig edge detected in {}".format(fname))
                            contig_edge = True
                    ...
                    # If translation isn't available translate manually, this will take longer
                        ...
                        if reminder > 0:
                            if fuzzy_start and fuzzy_end:
                                print(str(datetime.now()) + ": " + "Warning, CDS ({}, {}) has fuzzy\
        ...
        if bgc_size > min_bgc_size:  # exclude the bgc if it's too small
            ...
            if len(valid_classes & subproduct) == 0:
                if verbose:
                    print(str(datetime.now()) + ": " + " Skipping {} (type: {})".format(clusterName, product))
                return False
                ...
                if total_seq_length > 0:
                    ...
                    if save_fasta:
                        ...
                        for locus in del_list:
                            if verbose:
                                print(str(datetime.now()) + ": " + "   Removing {} because it overlaps with other ORF".format(locus))
            ...
            if verbose:
                print(str(datetime.now()) + ": " + "  Adding {} ({} bps)".format(fname, str(bgc_size)))
                                
        else:
            print(str(datetime.now()) + ": " + " Discarding {} (size less than {} bp, was {})".format(clusterName, str(min_bgc_size), str(bgc_size)))
...
def get_gbk_files(inputpath, outputdir, bgc_fasta_folder, min_bgc_size, exclude_gbk_str, bgc_info):
    ...
    if os.path.isfile(inputpath):
        if inputpath.endswith(".gbk"):
            files = [input_path]
        elif inputpath.endswith(".txt"):
            files = []
            with open(inputpath, "r") as handle:
                for line in handle:
                    line = line.rstrip()
                    if line.endswith(".gbk"):
                        files.append(line)
        else:
            sys.exit("Error: Unrecognizable file format.")
    else:
    ...
    for filepath in files:
        ...
        if type(exclude_gbk_str) == str and exclude_gbk_str != "" and \
            ...
            if verbose:
                print(str(datetime.now()) + ": " + " Skipping file " + fname)
            continue
            ...
            continue
        if "_ORF" in fname:
            print(str(datetime.now()) + ": " + " Skipping file {} (string '_ORF' is used internally)".format(fname))
            continue
    ...
    if len(files_no_proteins) > 0:
        print(str(datetime.now()) + ": " + "  Warning: Input set has files without protein sequences. They will be discarded")
        print(str(datetime.now()) + ": " + "   (See no_sequences_list.txt)")
    ...
    if len(files_no_biosynthetic_genes) > 0 and (mode == "glocal" or mode == "auto"):
        print(str(datetime.now()) + ": " + "  Warning: Input set has files with no Biosynthetic Genes (affects alignment mode)")
        print(str(datetime.now()) + ": " + "   See no_biosynthetic_genes_list.txt")
    ...
    print(str(datetime.now()) + ": " + " Files that had its sequence extracted: {:d}".format(processed_sequences))
...
def generate_dist_matrix(parms):
    ...
    try:
        ...
        print(str(datetime.now()) + ": " + " Warning: domain list for {} or {} was not found. Extracting from pfs files".format(cluster1, cluster2))
    ...
    if len(domain_list_A) == 0 or len(domain_list_B) == 0:
        print(str(datetime.now()) + ": " + "   Warning: Regarding distance between clusters {} and {}:".format(cluster1, cluster2))
        if len(domain_list_A) == 0 and len(domain_list_B) == 0:
            print(str(datetime.now()) + ": " + "   None have identified domains. Distance cannot be calculated")
        elif (domain_list_A) == 0:
            print(str(datetime.now()) + ": " + "   Cluster {} has no identified domains. Distance set to 1".format(cluster1))
        else:
            print(str(datetime.now()) + ": " + "   Cluster {} has no identified domains. Distance set to 1".format(cluster2))
...
def cluster_distance_lcs(A, B, A_domlist, B_domlist, dcg_A, dcg_b, core_pos_A, core_pos_b, go_A, go_b, bgc_class):
    ...
    #print(A, B)
    #print(str(datetime.now()) + ": " + "  "*offset_A + " ".join(map(str,dcg_A[:sliceStartA])) + "[" + " ".join(map(str,dcg_A[sliceStartA:sliceStartA+sliceLengthA])) + "]" + " ".join(map(str,dcg_A[sliceStartA+sliceLengthA:])) + "\t" + A )
    #print(str(datetime.now()) + ": " + "  "*offset_B + " ".join(map(str,dcg_B[:sliceStartB])) + "[" + " ".join(map(str,dcg_B[sliceStartB:sliceStartB+sliceLengthB])) + "]" + " ".join(map(str,dcg_B[sliceStartB+sliceLengthB:])) + "\t" + b_name)
    ...
    if mode=="glocal" or (mode=="auto" and (bgc_info[A].contig_edge or bgc_info[B].contig_edge)):
        ...
        if min(sliceLengthA, sliceLengthB) >= 5:
            ...
            #else:
                #print(str(datetime.now()) + ": " + " - - Not a valid overlap found - - (no biosynthetic genes)\n")
                
        #else:
                #print(str(datetime.now()) + ": " + " - - Not a valid overlap found - - (shortest slice not large enough)\n")
            ...
            for domsb in range(num_copies_b):
                ...
                try:
                    ...
                    if shared_domain not in missing_aligned_domain_files and verbose:
                        ...
                        print(str(datetime.now()) + ": " + "  Warning: {}.algn not found. Trying pairwise alignment...".format(shared_domain))
                ...
                if len(aligned_seqA) != len(aligned_seqB):
                    print(str(datetime.now()) + ": " + "\tWARNING: mismatch in sequences' lengths while calculating sequence identity ({})".format(shared_domain))
                    print(str(datetime.now()) + ": " + "\t  Specific domain 1: {} len: {}".format(sequence_tag_a, str(len(aligned_seqA))))
                    print(str(datetime.now()) + ": " + "\t  Specific domain 2: {} len: {}".format(sequence_tag_b, str(len(aligned_seqB))))
    ...
    if Distance < 0.0:
        if Distance < -0.000001: # this definitely is something else...
            print(str(datetime.now()) + ": " + "Negative distance detected!")
            print(Distance)
            print(str(datetime.now()) + ": " + "{} - {}".format(A, B))
            print(str(datetime.now()) + ": " + "J: {}\tDSS: {}\tAI: {}".format(str(Jaccard), str(DSS), str(AI)))
            print(str(datetime.now()) + ": " + "Jw: {}\tDSSw: {}\tAIw: {}".format(str(Jaccardw), str(DSSw), str(AIw)))
        Distance = 0.0
...
@timeit
def launch_mafft(cores, domain_sequence_list):
    for domain_file in domain_sequence_list:
        run_mafft(cores, domain_file)
        
        
def run_mafft(cores, domain_file):
    alignment_file = domain_file[:-6] + ".algn"
    
    mafft_cmd_list = []
    mafft_cmd_list.append("mafft") 
    mafft_cmd_list.append("--quiet")
    mafft_cmd_list.append("--auto")
    mafft_cmd_list.append("--thread")
    mafft_cmd_list.append(str(cores))
    mafft_cmd_list.append(domain_file)

    proc_mafft = subprocess.Popen(mafft_cmd_list, shell=False, stdout=PIPE, stderr=PIPE)
    out, err = proc_mafft.communicate()
...
def runHmmScan(fastaPath, hmmPath, outputdir, verbose):
    ...
    if os.path.isfile(fastaPath):
        ...
        if verbose == True:
            print(str(datetime.now()) + ": " + "   " + hmmscan_cmd)
...
def parseHmmScan(hmmscanResults, pfd_folder, pfs_folder, overlapCutoff):
    ...
    if os.path.isfile(hmmscanResults):
        ...
        if num_domains > 0:
            if verbose:
                print(str(datetime.now()) + ": " + "  Processing domtable file: " + outputbase)
            ...
            print(str(datetime.now()) + ": " + "  No domains where found in {}.domtable. Removing it from further analysis".format(outputbase))
...
def clusterJsonBatch(bgcs, pathBase, className, matrix, pos_alignments, cutoffs=[1.0], damping=0.9, clusterClans=False, clanCutoff=(0.5,0.8), htmlFolder=None):
    ...
    for cutoff in cutoffs:    
        ...
        print(str(datetime.now()) + ": " + "  Cutoff: {}".format(cutoff))
        ...
        for exemplar_idx in familiesDict:
            ...
            if len(tree_domains) == 1:
                if verbose:
                    print(str(datetime.now()) + ": " + "  Warning: core shared domains for GCF {} consists of a single domain ({})".format(exemplar_idx, [x for x in tree_domains][0]))
            ...
            for domain in tree_domains:
                ...
                for bgc in alignments:
                    ...
                    if bgc == exemplar_idx:
                        ...
                        if verbose:
                            print(str(datetime.now()) + ": " + "   BGC {} ({}) does not share a common domain core (GCF: {}, domain: {})".format(clusterNames[bgc], bgc, exemplar_idx, domain))
            ...
            if verbose:
                print(str(datetime.now()) + ": " + "  Working GCF {}, cutoff {}".format(exemplar_idx, cutoff))
            ...
            if not os.path.isfile(newick_file_path) or os.path.getsize(newick_file_path) == 0:
                ...
                with open(newick_file_path,"r") as newick_file:
                    try:
                        ...
                        print(str(datetime.now()) + ": " + " Warning! There was an error while reading tree file {}".format(newick_file))
                        print(str(e))
                        ...
                        try:
                            ...
                            if verbose:
                                print(str(datetime.now()) + ": " + " Warning: Unable to root at midpoint file {}".format(newick_file_path))
                            pass
        ...
        if verbose:
            print(str(datetime.now()) + ": " + "  Writing clustering file")
        ...
        if len(clanLabels) > 0:
            if verbose:
                print(str(datetime.now()) + ": " + "   Writing Clans file")
...
if __name__=="__main__":
    ...
    if options.outputdir == "":
        print(str(datetime.now()) + ": " + "please provide a name for an output folder using parameter -o or --outputdir")
        sys.exit(0)
    ...
    if os.path.isfile(options.anchorfile):
        ...
        print(str(datetime.now()) + ": " + "File with list of anchor domains not found")
        anchor_domains = set()
    ...
    if not (os.path.isfile(h3f) and os.path.isfile(h3i) and os.path.isfile(h3m) and os.path.isfile(h3p)):
        print(str(datetime.now()) + ": " + "One or more of the necessary Pfam files (.h3f, .h3i, .h3m, .h3p) were not found")
        if os.path.isfile(os.path.join(pfam_dir, "Pfam-A.hmm")):
            print(str(datetime.now()) + ": " + "Please use hmmpress with Pfam-A.hmm")
        else:
            print(str(datetime.now()) + ": " + "Please download the latest Pfam-A.hmm file from http://pfam.xfam.org/")
            print(str(datetime.now()) + ": " + "Then use hmmpress on it, and use the --pfam_dir parameter to point to the location of the files")
        sys.exit()
    ...
    if options.domain_whitelist:
        ...
        if os.path.isfile(os.path.join(bigscape_path,"domain_whitelist.txt")):
            ...
            if len(domain_whitelist) == 0:
                print(str(datetime.now()) + ": " + "Error: --domain_whitelist used, but no domains found in the file")
            else:
                has_whitelist = True
        else:
    ...
    if type(exclude_gbk_str) == str and exclude_gbk_str != "":
        print(str(datetime.now()) + ": " + " Skipping files with '{}' in their filename".format(exclude_gbk_str))
    # Exclude from list of strings
    elif type(exclude_gbk_str) == list and exclude_gbk_str != []:
        print(str(datetime.now()) + ": " + " Skipping files with one or more of the following strings in \
        ...
        if numbgcs == 0:
            ...
            with zipfile.ZipFile(os.path.join(mibig_path,mibig_zipfile_numbgcs[0]), 'r') as mibig_zip:
                for fname in mibig_zip.namelist():
                    ...
                    if verbose:
                        print(str(datetime.now()) + ": " + "  Extracted {}".format(extractedbgc))
        
        elif mibig_zipfile_numbgcs[2] == numbgcs:
            print(str(datetime.now()) + ": " + "  MIBiG BGCs seem to have been extracted already")
        else:
    ...
    if force_hmmscan:
        ...
        task_set = fastaFiles
        print(str(datetime.now()) + ": " + " Forcing domain prediction on ALL fasta files (--force_hmmscan)")
    else:
        ...
        if len(task_set) == 0:
            print(str(datetime.now()) + ": " + " All fasta files had already been processed")
        elif len(alreadyDone) > 0:
            if len(task_set) < 20:
                print(str(datetime.now()) + ": " + " Warning! The following NEW fasta file(s) will be processed: {}".format(", ".join(".".join(x.split(os.sep)[-1].split(".")[:-1]) for x in task_set)))
            else:
                print(str(datetime.now()) + ": " + " Warning: {} NEW fasta files will be processed".format(str(len(task_set))))
        else:
            print(str(datetime.now()) + ": " + " Predicting domains for {} fasta files".format(str(len(fastaFiles))))
    ...
    pool.close()
    pool.join()
    print(str(datetime.now()) + ": " + " Finished generating domtable files.")
    ...
    if len(domtableFilesUnprocessed) == 0: # Re-run
        print(str(datetime.now()) + ": " + " All domtable files had already been processed")
    elif len(alreadyDone) > 0: # Incomplete run
        if len(domtableFilesUnprocessed) < 20:
            print(str(datetime.now()) + ": " + " Warning! The following domtable files had not been processed: {}".format(", ".join(".".join(x.split(os.sep)[-1].split(".")[:-1]) for x in domtableFilesUnprocessed)))
        else:
            print(str(datetime.now()) + ": " + " Warning: {} domtable files will be processed".format(str(len(domtableFilesUnprocessed))))
    else: # First run
        print(str(datetime.now()) + ": " + " Processing {} domtable files".format(str(len(domtableFiles))))
    ...
    if len(baseNames - set(pfd.split(os.sep)[-1][:-9] for pfd in alreadyDone)) == 0:
        ...
        print(str(datetime.now()) + ": " + " New domain sequences to be added; cleaning domains folder")
        for thing in os.listdir(domains_folder):
            os.remove(os.path.join(domains_folder,thing))

    print(str(datetime.now()) + ": " + " Finished generating pfs and pfd files.")
    ...
    filtered_matrix = []
    if options.skip_ma:
        print(str(datetime.now()) + ": " + " Running with skip_ma parameter: Assuming that the domains folder has all the fasta files")
        try:
        ...
        print(str(datetime.now()) + ": " + " Adding sequences to corresponding domains file")
            
        for outputbase in baseNames:
            if verbose:
                print(str(datetime.now()) + ": " + "   Processing: " + outputbase)
    ...
    print(str(datetime.now()) + ": " + " Reading the ordered list of domains from the pfs files")
    for outputbase in baseNames:
    ...
    print(str(datetime.now()) + ": " + " Creating arrower-like figures for each BGC")
    
    # read hmm file. We'll need that info anyway for final visualization
    print(str(datetime.now()) + ": " + "  Parsing hmm file for domain information")
    pfam_info = {}
    ...
    if len(working_set) > 0:
        ...
        print(str(datetime.now()) + ": " + "  Reading BGC information and writing SVG")
        ...
        print(str(datetime.now()) + ": " + "  All SVG from the input files seem to be in the SVG folder")
    
    availableSVGs.clear()
    print(str(datetime.now()) + ": " + " Finished creating figures")
    ...
    if not options.skip_ma:
        print(str(datetime.now()) + ": " + "Performing multiple alignment of domain sequences")
        ...
        if try_MA_resume:
            ...
            if len(temp_aligned) > 0:
                print(str(datetime.now()) + ": " + " Found domain fasta files without corresponding alignments")
        ...
        for domain_file in domain_sequence_list_temp:
            ...
            if len(sequence_tag_list) == 1:
                ...
                if verbose:
                    print(str(datetime.now()) + ": " + " Skipping Multiple Alignment for {} (appears only in one BGC)".format(domain_name))
        ...
        if len(domain_sequence_list) > 0:
            print(str(datetime.now()) + ": " + "\n Using mafft")
            launch_mafft(cores, domain_sequence_list)
                
            # verify all tasks were completed by checking existance of alignment files
            for domain_file in domain_sequence_list:
                if not os.path.isfile(domain_file[:-6]+".algn"):
                    print(str(datetime.now()) + ": " + "   ERROR, {}.algn could not be found (possible issue with aligner).".format(domain_file[:-6]))
                    stop_flag = True
            if stop_flag:
                sys.exit()
    ...
    print(str(datetime.now()) + ": " + " Trying to read domain alignments (*.algn files)")
    ...
    print(str(datetime.now()) + ": " + "   Writing the complete Annotations file for the complete set")
    ...
    if options_mix:
        ...
        print(str(datetime.now()) + ": " + "  Calculating all pairwise distances")
        if has_query_bgc:
        ...
        if has_query_bgc:
            ...
            print(str(datetime.now()) + ": " + "   Writing annotation file")
            ...
            print(str(datetime.now()) + ": " + "   Removing {} non-relevant MIBiG BGCs".format(len(mibig_set_del)))
            mix_set_idx = 0
            bgc_to_mix_set_idx = {}
        ...
        print(str(datetime.now()) + ": " + "  Writing output files")
        ...
        print(str(datetime.now()) + ": " + "  Calling Gene Cluster Families")
        reduced_network = []
        pos_alignments = {}
    ...
    if options_classify:
        ...
        print(str(datetime.now()) + ": " + "  Sorting the input BGCs\n")
        ...
        for bgc_class in BGC_classes:
            ...
            if use_relevant_mibig:
                if len(set(BGC_classes[bgc_class]) & mibig_set_indices) == len(BGC_classes[bgc_class]):
                    print(str(datetime.now()) + ": " + " - All clusters in this class are MIBiG clusters -")
                    print(str(datetime.now()) + ": " + "  If you'd like to analyze MIBiG clusters, turn off the --mibig option")
                    print(str(datetime.now()) + ": " + "  and point --inputdir to the Annotated_MIBiG_reference folder")
                    continue
            ...
            print(str(datetime.now()) + ": " + "   Writing annotation files")
            ...
            if has_query_bgc:
                ...
                # Create an additional file with the list of all clusters in the class + other info
                # This version of the file only has information on the BGCs connected to Query BGC
                print(str(datetime.now()) + ": " + "   Writing annotation file (Query BGC)")
