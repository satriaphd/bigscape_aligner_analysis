...
@timeit
def launch_muscle(cores, domain_sequence_list):
    """
    Launches instances of Muscle with multiprocessing.
    Note that the domains parameter contains the .fasta extension
    """
    pool = Pool(cores, maxtasksperchild=32)
    pool.map(run_muscle, domain_sequence_list)
    pool.close()
    pool.join()
...
if __name__=="__main__":
    ...
    if not options.skip_ma:
        ...
        if len(domain_sequence_list) > 0:
            print(str(datetime.now()) + ": " + "\n Using muscle")
            launch_muscle(cores, domain_sequence_list)
